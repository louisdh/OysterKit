    ...
    func rule(from grammar:STLRIntermediateRepresentation, creating token:Token, inContext context:GenerationContext, annotations: RuleAnnotations)->Rule? {
        ...
        case .choice(let elements):
            ...
            if scannable {
                ...
                let choiceToken = STLRIntermediateRepresentation.DynamicToken(rawValue: 0, name: elements.flatMap({"\($0)"}).joined(separator: "|"))
...
fileprivate extension STLRIntermediateRepresentation.Quantifier{
    func wrapped(token:Token)->Token{
        switch self{
        case .one:
            return token
        case .not:
            return STLRIntermediateRepresentation.DynamicToken.init(rawValue: 0, name: "!\(token)")
        case .consume:
            return STLRIntermediateRepresentation.DynamicToken.init(rawValue: 0, name: "\(token)-")
        case .zeroOrOne:
            return STLRIntermediateRepresentation.DynamicToken.init(rawValue: 0, name: "\(token)?")
...
extension STLRIntermediateRepresentation.Element{
    func rule(from grammar:STLRIntermediateRepresentation, creating token:Token, inContext context:GenerationContext, annotations:RuleAnnotations)->Rule? {
        ...
        case .terminal(let terminal, _,let lookahead, _):
            guard let r = terminal.rule(from: grammar, creating: token, with: mergedElementAnnotations) else {
                return nil
            }
            if lookahead {
                return ParserRule.lookahead(r, mergedElementAnnotations)
            }
            return quantifier.rule(appliedTo: r, producing: quantifier.wrapped(token: token), quantifiersAnnotations: mergedQuantifierAnnotations)
        case .group(let expression, _,let lookahead, _):
            let believeCorrect = STLRIntermediateRepresentation.DynamicToken(rawValue: 0, name: "(\(expression))")
            ...
            return quantifier.rule(appliedTo: r, producing: quantifier.wrapped(token: believeCorrect), quantifiersAnnotations: mergedQuantifierAnnotations)
        case .identifier(let identifier, _,let lookahead, _):
            ...
            var finalRule = quantifier.rule(appliedTo: r, producing: quantifier.wrapped(token: identifier.token), quantifiersAnnotations: mergedQuantifierAnnotations)
    ...
    public var token : Token {
        ...
        case .terminal(let terminal, _,let lookahead,_) where lookahead == false:
            return STLRIntermediateRepresentation.DynamicToken(rawValue: 0, name: "\(terminal)")
        default:
            return STLRIntermediateRepresentation.DynamicToken(rawValue: 0)
        }
    }
