...
extension STLRIntermediateRepresentation : ASTNodeConstructor {
    ...
    private func createElement(from node:ValuedNode, _ quantifier:Quantifier, _ lookahead:Bool, _ annotations: ElementAnnotations)->Element{
    ...
    public func match(token: Token, context: LexicalContext, children: [HeterogeneousNode]) -> HeterogeneousNode? {
        ...
        case .group:
            ...
            return HeterogeneousNode(for: token, at: matchRange, value: child.value)
        case .expression:
            ...
            return HeterogeneousNode(for: token, at: matchRange, value: Expression.element(element))
        case .rule:
            ...
            rules.append(rule)
            return HeterogeneousNode(for: token, at: matchRange, value: rule)
        case .characterRange:
            ...
            if upper<lower{
                return HeterogeneousNode(for: STLR.characterSet, at: matchRange, value: TerminalCharacterSet.customRange(CharacterSet(charactersIn: upper...lower), start: upper, end: lower))
            }
            
            return HeterogeneousNode(for: STLR.characterSet,at: matchRange , value: TerminalCharacterSet.customRange(CharacterSet(charactersIn: lower...upper), start: lower, end: upper))
        case .element:
            //Newer, cleaner, more robust syntax for accessing AST
            var annotations : ElementAnnotations    = children[STLR.annotations]?.cast() ?? []
            let lookahead   : Bool                  = children[STLR.lookahead]?.cast() ?? false
            let transient   : Bool                  = children[STLR.transient]?.cast() ?? false
            var negated     : Quantifier            = children[STLR.negated]?.cast() ?? .one
            var quantifier  : Quantifier            = children[STLR.quantifier]?.cast() ?? .one
            ...
            if let inlineIdentifier = annotations[annotation: ElementAnnotation.token], case let ElementAnnotationValue.string(stringValue) = inlineIdentifier {
                ...
                if identifiers[stringValue] != nil {
                    identifier = identifiers[stringValue]!
                } else {
            ...
            if quantifier != .one && lookahead {
                ...
                //Wrap in a lookahead group
                element = Element.group(Expression.element(element), Quantifier.one, true, annotations)
            }
            
            return HeterogeneousNode(for: token,at: matchRange , value: element)
        case .sequence, .choice:
            ...
            if case .sequence = token {
                return HeterogeneousNode(for: STLR.expression,at: matchRange , value: STLRIntermediateRepresentation.Expression.sequence(elements))
            } else {
                return HeterogeneousNode(for: STLR.expression,at: matchRange , value: STLRIntermediateRepresentation.Expression.choice(elements))
            }
        case .characterSetName:
            ...
            return HeterogeneousNode(for: STLR.characterSet,at: matchRange , value: characterSet)
        case .terminal:
            ...
            case .characterSet:
                ...
                return HeterogeneousNode(for: STLR.terminal,at: matchRange , value: STLRIntermediateRepresentation.Terminal(with: characterSet))
            case .stringBody:
                guard let string : String = children[0].cast() else {
                    fatalError("Expected a string")
                }
                return HeterogeneousNode(for: STLR.terminal,at: matchRange , value: STLRIntermediateRepresentation.Terminal(with: string))
            default:
                return nil
            }
        case .identifier:
            return HeterogeneousNode(for: token,at: matchRange , value: getIdentifier(named: context.matchedString, at: context.range))
        case .lookahead, .transient:
            return HeterogeneousNode(for: token,at: matchRange , value: true)
        case .negated:
            return HeterogeneousNode(for: token,at: matchRange , value: STLRIntermediateRepresentation.Quantifier(from: context.matchedString))
        case .quantifier:
            return HeterogeneousNode(for: token,at: matchRange , value: STLRIntermediateRepresentation.Quantifier(from: context.matchedString))
        case .annotations:
            ...
            return HeterogeneousNode(for: token,at: matchRange , value: annotations)
        case .annotation:
            ...
            return HeterogeneousNode(for: token,at: matchRange , value: ElementAnnotationInstance(annotation, value: value ?? ElementAnnotationValue.set))
        case .literal:
            if let value : Bool = children[0].cast() {
                return HeterogeneousNode(for: token,at: matchRange , value: ElementAnnotationValue.bool(value))
            } else if let value : Int = children[0].cast() {
                return HeterogeneousNode(for: token,at: matchRange , value: ElementAnnotationValue.int(value))
            } else if let value : String = children[0].cast(){
                return HeterogeneousNode(for: token,at: matchRange , value: ElementAnnotationValue.string(value))
            } else {
                fatalError("Unknown literal type for \(children[0])")
            }
        case .number:
            return HeterogeneousNode(for: token,at: matchRange , value: Int.init(context.matchedString) ?? 0)
        case .boolean:
            return HeterogeneousNode(for: token,at: matchRange , value: context.matchedString == "true")
        case .label:
            return HeterogeneousNode(for: token,at: matchRange , value: ElementAnnotation(rawValue: context.matchedString))
        case .stringBody, .terminalBody:
            return HeterogeneousNode(for: STLR.stringBody,at: matchRange , value: context.matchedString)
        default:
            return nil
        }
    }
