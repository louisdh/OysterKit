...
private struct LexerContext : LexicalContext {
    ...
    var matchedString : String {
        return "\(source.unicodeScalars[range])"
    }
...
public class Lexer : LexicalAnalyzer, CustomStringConvertible{
    ...
    public func proceed() -> LexicalContext {
        let mark = marks.removeLast()
    ...
    public func scan(terminal: String) throws {
    ...
    public func scan(oneOf: CharacterSet) throws {
        if !scanner.scan(characterFrom: oneOf) {
            throw GrammarError.matchFailed
        }
    }
    
    public func scanUpTo(terminal:String) throws {
        if !scanner.scanUpTo(string: terminal){
            throw GrammarError.matchFailed
        }
    }
    
    public func scanUpTo(oneOf terminal:CharacterSet) throws {
        if !scanner.scanUpTo(characterFrom: terminal){
            throw GrammarError.matchFailed
        }
    }
    
    public func scanNext() throws {
