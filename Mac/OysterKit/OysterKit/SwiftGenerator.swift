...
//  OysterKit
//
//  Created by Nigel Hughes on 08/08/2016.
...
public extension STLRIntermediateRepresentation {
    func swift(grammar name:String, colorPrefix : String = "NS", colors : [String : (r:Float,g:Float,b:Float)]?  = nil)->String?{
        ...
        //
        output.add(line: "")
        output.add(depth: 1, line:      "case _transient, "+self.rules.flatMap({$0.identifier}).map({"\($0)"}).joined(separator: ", "))
        ...
        output.add(depth: 1, line:      "public static func parse(source: String) -> HeterogeneousAST {")
...
internal extension STLRIntermediateRepresentation.Quantifier{
