...
//  OysterKit
//
//  Created by Nigel Hughes on 20/07/2016.
...
class DynamicGeneratorTest: XCTestCase {
    ...
    func testComment(){
        guard let rule = "\"\\\\\\\\\" (.newlines!)* .newlines".dynamicRule(token: TT.comment) else {
            XCTFail("Could not compile")
            return
        }
    ...
    func testTerminalChoice(){
        do {
            ...
            XCTAssert(result == "tokenA =  (@error(\"error\")(\"a\" | \"b\" | \"c\"))", "Bad generated output '\(result)'")
        } catch (let error){
            XCTFail("\(error)")
        }
    }
    ...
    func testAnnotationOnQuantifier(){
        do {
            ...
            XCTAssert(result == "tokenA =  (@error(\"Expected a letter\") tokenA =  .characterSet+)", "Bad generated output '\(result)'")
        } catch (let error){
            XCTFail("\(error)")
        }
    }
